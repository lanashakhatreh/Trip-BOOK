...
if( ! class_exists( 'Travel_Company_Helper_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Travel_Company_Helper_Control_Repeater extends WP_Customize_Control {
        ...
        public function __construct( $manager, $id, $args = array() ) {
            ...
            $this->row_label = array(
                'type' => 'text',
                'value' => esc_attr__( 'row', 'travel-company-helper' ),
                'field' => false,
            );
            ...
            if ( empty( $this->button_label ) ) {
                $this->button_label = sprintf( esc_attr__( 'Add new %s', 'travel-company-helper' ), $this->row_label['value'] );
            }
        ...
        public function enqueue() {    
            ...
            wp_enqueue_script( 'blossom-feminine-repeater', get_template_directory_uri() . '/assets/customizer/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
            wp_enqueue_style( 'blossom-feminine-repeater', get_template_directory_uri() . '/assets/customizer/repeater.css', null );
        }
        ...
        protected function render_content() {
            ...
            <?php if ( isset( $this->choices['limit'] ) ) : ?>
                <p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'travel-company-helper' ), esc_html( $this->choices['limit'] ) ); ?></p>
            <?php endif; ?>
            ...
            <script type="text/html" class="customize-control-repeater-content">
                ...
                <li class="repeater-row minimized" data-row="{{{ index }}}">
                    ...
                    <div class="repeater-row-content">
                        <# _.each( data, function( field, i ) { #>
                            ...
                            <div class="repeater-field repeater-field-{{{ field.type }}}">
                                ...
                                <# } else if ( 'color' === field.type ) { #>
                                    ...
                                    <label>
                                        ...
                                        <# if ( field.description ) { #>
                                            <span class="description customize-control-description">{{{ field.description }}}</span>
                                        <# } #>
                                        <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'travel-company-helper' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
                                ...
                                <# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
                                    ...
                                    <figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'travel-company-helper' ); ?>" >
                                        <# if ( field.default ) { #>
                                            <# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
                                            <img src="{{{ defaultImageURL }}}">
                                        <# } else { #>
                                            <?php esc_attr_e( 'No Image Selected', 'travel-company-helper' ); ?>
                                        <# } #>
                                    </figure>
    
                                    <div class="actions">
                                        <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'travel-company-helper' ); ?></button>
                                        <button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'travel-company-helper' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'travel-company-helper' ); ?>" >
                                            <# if ( field.default ) { #>
                                                <?php esc_attr_e( 'Change Image', 'travel-company-helper' ); ?>
                                            <# } else { #>
                                                <?php esc_attr_e( 'Add Image', 'travel-company-helper' ); ?>
                                            <# } #>
                                        </button>
                                        <# if ( field.default.id ) { #>
                                ...
                                <# } else if ( field.type === 'upload' ) { #>
                                    ...
                                    <figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'travel-company-helper' ); ?>" >
                                        <# if ( field.default ) { #>
                                        ...
                                        <# } else { #>
                                            <?php esc_attr_e( 'No File Selected', 'travel-company-helper' ); ?>
                                        <# } #>
                                    </figure>
    
                                    <div class="actions">
                                        <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
                                        <button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'travel-company-helper' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'travel-company-helper' ); ?>" >
                                            <# if ( field.default ) { #>
                                                <?php esc_attr_e( 'Change File', 'travel-company-helper' ); ?>
                                            <# } else { #>
                                                <?php esc_attr_e( 'Add File', 'travel-company-helper' ); ?>
                                            <# } #>
                                        </button>
                                        <# if ( field.default.id ) { #>
                                            <input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
                        ...
                        <# }); #>
                        <button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'travel-company-helper' ); ?></button>
                    </div>
                </li>
            </script>
            <?php
